...
class StudentChoose(tk.Frame):
    def __init__(self, master, group, subject):
        ...
        if subject == None: #for student manager
            ...
            tk.Button(self, text="Dodaj", command=lambda: self.master.navigate_to(Student_Add, self.group), height=2).grid(row = 2, column = 0, sticky=N+E+S+W, pady=3, padx=3)
            tk.Button(self, text="Edytuj", command= self.try_edit, height=2).grid(row = 2, column = 1, sticky=N+E+S+W, pady=3, padx=3)
            tk.Button(self, text="Usuń", command= self.try_delete, height=2).grid(row = 2, column = 2, sticky=N+E+S+W, pady=3, padx=3)
