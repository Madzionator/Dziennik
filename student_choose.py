...
class StudentChoose(tk.Frame):
    def __init__(self, master, group, subject):
        ...
        if subject == None: #for student manager
            ...
            tk.Button(self, text="➕ Dodaj studenta", command=lambda: self.master.navigate_to(Student_Add, self.group), height=2).grid(row = 2, column = 0, sticky=N + E + S + W, pady=3, padx=3)
            tk.Button(self, text="✏ Edytuj studenta", command= self.try_edit, height=2).grid(row = 2, column = 1, sticky=N + E + S + W, pady=3, padx=3)
            tk.Button(self, text="❌ Usuń studenta", command= self.try_delete, height=2).grid(row = 2, column = 2, sticky=N + E + S + W, pady=3, padx=3)
            ...
            def StudentGrades(event):
                self.student_choice = self.get_choice() 
                if self.student_choice != 0:
                    self.master.navigate_to(Grade_Manager, self.student_choice, self.subject)

            self.student_list.bind('<<ListboxSelect>>', StudentGrades)
            self.student_list.bind('<Double-1>', StudentGrades)

        tk.Button(self, text="⬅ Wróc", command=lambda: master.go_back(), height = 2).grid(row = 3, column = 0, sticky=N + E + S + W, pady=3, padx=3)
        for i in range(0, 3):
