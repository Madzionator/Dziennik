...
class StartPage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Button(self, text="✏ Edytuj przedmiot", command= self.try_edit, height=2).grid(row=2, column=1, sticky=N + E + S + W, pady=3, padx=3)
        tk.Button(self, text="➕ Dodaj nowy przedmiot", command=lambda: master.navigate_to(Subject_Add), height=2).grid(row=3, column=0, sticky=N + E + S + W, pady=3, padx=3)
        tk.Button(self, text="❌ Usuń przedmiot", command=self.delete_subject, height=2).grid(row=3, column=1, sticky=N + E + S + W, pady=3, padx=3)
